\chapter{Matematično izražanje}\label{POGLAVJE: Matematično izražanje}
	...
	\section{Simbolni zapis}\label{RAZDELEK: Simbolni zapis}
		...
		\begin{opomba}
			Logiki, kjer sta edini resničnostni vrednosti resnica in neresnica, rečemo \df{klasična logika}. Obstajajo splošnejše vrste logike, kjer je $\set{\true, \false}$ prava podmnožica $\tvs$, ampak v tej knjigi se bomo omejili na klasično logiko, na katero ste navajeni in ki se uporablja v večjem delu matematike.
		\end{opomba}
		...
		\note{dokončaj resničnostne tabele}
