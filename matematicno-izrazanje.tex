\chapter{Matematično izražanje}\label{POGLAVJE: Matematično izražanje}
	...
	\section{Preslikave}
		...
		\note{Sklop (kompozicija, kompozitum) preslikav. Razčlenitev (dekompozicija, faktorizacija) preslikav?}
		
		\davorin{Definirati moramo tudi oznako $\set{f(x)}{x \in X}$, kar je druge vrste oznaka kot prej definirana $\set{x \in X}{\phi(x)}$. Se gremo primerjavo s Pythonom (razlika med \texttt{\{f(x) for x in X\}} in \texttt{\{x if phi(x)\}})? Smo matematični hipsterji in uvedemo oznako $\{f(x) \,|\, x \in X \,|\, \phi(x)\}$, ki ustreza \texttt{\{f(x) for x in X if phi(x)\}}?}
