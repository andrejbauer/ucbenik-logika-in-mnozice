...
\begin{dokaz}
  ...
  \item Naj bosta $f : A \to B$ in $g : B \to C$ preslikavi in $g \circ f$ epimorfizem.
    ...
    velja $h \circ g = k \circ g$. Dokazujemo, da je $h = k$. Iz $h \circ g = k \circ g$
...
\begin{dokaz}
  ...
  \item
    ...
    \end{equation*}
    % a
    ter preslikavi $\chi_S : B \to \two$ in $(y \mapsto \top) : B \to \two$. Ker velja
...
\begin{izrek}
  ...
  %
  \begin{itemize}
  \item praslike so monotone: če je $S \subseteq T \subseteq B$, potem je $\invimg{f}(S) \subseteq \invimg{f}(T)$
  \item slike so monotone: če je $X \subseteq Y \subseteq A$, potem je $\img{f}(X) \subseteq \img{f}(Y)$.
  \end{itemize}
\end{izrek}
...
\begin{izrek}
  ...
  \begin{equation*}
    \textstyle
     ...
     \iinn
     \img{f} (\bigcap_{i \in I} T_i) \subseteq \bigcap_{i \in I} \img{f}(T_i).
  \end{equation*}
  %
\end{izrek}
