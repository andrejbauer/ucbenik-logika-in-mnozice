...
Osnovni gradniki logičnih formul so \df{logične operacije}, ki jih delimo na tri sklope.
V prvi sklop sodita \df{logični konstanti}:
%
\begin{itemize}
\item resnica $\top$,
\item neresnica $\bot$.
...
neresnico $\bot$ z $0$ ali \texttt{False}. Naslednji sklop so \df{logični
vezniki}, s katerimi sestavljamo nove izjave iz že danih:
%
\begin{itemize}
\item \df{konjunkcija} $\phi \land \psi$, beremo \nls{$\phi$ in $\psi$},
\item \df{disjunkcija} $\phi \lor \psi$, beremo \nls{$\phi$ ali $\psi$},
\item \df{implikacija} $\phi \lthen \psi$, beremo na več načinov:
  %
  \begin{itemize}
  \item \nls{če $\phi$ potem $\psi$},
  \item \nls{iz $\phi$ sledi $\psi$},
  ...
  \item \nls{$\psi$ je potreben pogoj za $\phi$}.
  \end{itemize}
  %
\item \df{ekvivalenca} $\phi \liff \psi$, beremo \nls{$\phi$ če, in samo če, $\psi$} ali \nls{$\phi$ natanko tedaj, kadar~$\psi$},
\item negacija $\lnot \phi$, beremo \nls{ne $\phi$}, \nls{ni res, da $\phi$} ali \nls{ne velja $\phi$}.
\end{itemize}
%
...
\df{Neomejena kvantifikatorja} $\all{x} \phi$ in
