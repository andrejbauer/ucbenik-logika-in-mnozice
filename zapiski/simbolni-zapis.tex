...
\end{itemize}
%
V matematičnem govoru boditi pozorni na razliko med inkluzivno in ekskluzivno disjunkcijo, saj ju v vsakdanjem govoru pogosto mešamo ali ne ločimo med njima.
...
V tretji sklop sodita \df{logična kvantifikatorja}:
%
\begin{itemize}
\item univerzalni kvantifikator $\all{x \in S} \phi$, beremo \nls{za vse $x$
  iz $S$ velja $\phi$},
\item eksistenčni kvantifikator $\some{x \in S} \phi$, beremo \nls{obstaja
  $x$ v $S$, da velja $\phi$} ali \nls{za neki $x$ iz $S$ velja $\phi$}.
\end{itemize}
%
...
\begin{vaja}
  Je \nls{$n$ je sod in $n > 2$} \textbf{potreben} ali \textbf{zadosten}
  pogoj za \nls{$n$ ni praštevilo}?
\end{vaja}
