...
\section{Pravilo ekstenzionalnosti in končne množice}
...
V teoriji množic lahko prirejanje med elementi domene in kodomene podamo na kak drug način, ki ga ne moremo neposredno izraziti z matematično formulo.

\begin{zgled}
  \label{zgled:prirejanje-opis}
  Preslikava $f : \RR \to \RR$ je podana s prirejanjem, ki elementu $x \in \RR$ iz domene priredi najmanjše realno število $y \in \RR$ iz kodomene, za katerega velja $y^5 + y = x$.
\end{zgled}

Takoj se moramo vprašati, ali prirejanje v zgledu sploh poda preslikavo? Ali je res, da za vsak~$x \in \RR$ obstaja natanko en~$y \in \RR$, ki ima lastnost: najmanjše realno število~$y$, ki zadošča $x y^5 + y = x$. Odgovor je pritrdilen: $y$ obstaja, ker je $x y^5 + y - x$ polinom lihe stopnje v spremenljivki~$y$ in ima vsaj eno realno ničlo, po drugi strani pa je $y$ enolično določen, ker je ena od končno mnogo ničel tega polinoma najmanjša.
%
Če bi v zgledu potenco~$5$ nadomestili s~$6$, bi zgled pokvarili. Znate to utemeljiti?

Zapomnimo si, da je prirejanje med domeno~$X$ in kodomeno~$Y$ podano pravilno, če zadošča naslednjima pogojema:
%
\begin{itemize}
...
pravimo \df{parametri}. Predpis \nls{pomnoži z~$a$ in prištej~$b$} zapišemo
%
\begin{equation*}
  x \mapsto a \cdot x + b,
\end{equation*}
%
tu sta $a$ in $b$ parametra in $x$ vezana spremenljivka.
...
\end{equation*}
%
Mi se bomo držali zapisa z $\dfeq$, da bomo lahko ločili med definicijo~$f$ in trditvijo, da je $f(x)$ enako neki vrednosti.\footnote{Naslednji primer pojasni, v čem je razlika med $\dfeq$ in $=$: ``Naj bo preslikava $f : \RR \to \RR$ definirana s predpisom $f(x) \dfeq 1 + x^2$. Tedaj za vse $x \in \RR$ velja $f(x) = (x + 1)^2 - 2 x$.''}
...
poskusite sami.
%Nekaj prirejanj $g$ prikazuje naslednja razpredelnica:
%
% \begin{align*}
...
je treba vse preslikave vedno poimenovali in se nanje skliceval z njihovim imenom.\footnote{Če bi veljalo enako tudi za števila, vam v srednji šoli ne bi pustili pisati kar $3 + 5$, nujno bi bilo poimenovanje $a \dfeq 3 + 5$. Tudi trikotnika ne bi smeli narisati, ne da bi mu dali simbolno ime.}
%
...
Zmeda izhaja iz neprimernega zapisa preslikav, ko na primer rečemo \nls{$\cos x$ je soda funkcija}, čeprav bi bilo pravilno \nls{$\cos$ je soda funkcija}.
%
Če dosledno uporabljamo funkcijske predpise in vezane spremenljivke, lažje razumemo, da sta $\cos$ in $x \mapsto \cos x$ enaki preslikavi, zahvaljujoč pravilu ekstenzionalnosti, obe pa sta različni $\cos x$, ki sploh ni funkcija, ampak realno število, odvisno od parametra~$x$.

V bran tradicionalnemu zapisu pa moramo vseeno povedati, da se lahko \emph{dogovorimo} za
nekoliko napačen zapis, če to ne povzroča zmede. S tem se izkušeni matematik izogne preveč birokratskemu
pisanju nebistvenih podrobnosti in lahko bolj učinkovito komunicira s stanovskimi kolegicami in kolegi.
...
\item pravila \df{vpeljave} elementov, ki podajo operacije, s katerimi gradimo njene elemente,
\item pravila \df{uporabe}, ki podajo opreacije, s katerimi razgradimo ali uporabimo elemente,
\item \df{enačbe}, ki opredeljujejo zakonitosti podanih operacij.
\end{enumerate}
%
