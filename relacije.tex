\chapter{Relacije}\label{POGLAVJE: Relacije}
	...
	\section{Splošno o relacijah}
		...
		\begin{definicija}
			\df{Dvojiška} (ali s tujko \df{binarna}) \df{relacija} $\rel$ med elementi množic $X$ in $Y$ je podmnožica produkta $X \times Y$, skupaj s podatkom o $X$ in $Y$. Za takšno relacijo definiramo:
			\begin{itemize}
				\item
				...
				\item
					\df{definicijsko območje} ali \df{nosilec} relacije $\rel$ je množica $\dd{\rel} \dfeq \set{x \in X}{\xsome{y}[Y]{x \rel y}}$ (torej $\dd{\rel} \subseteq \dom(\rel)$),
				\item
					\df{zaloga vrednosti} ali \df{slika} relacije $\rel$ je množica $\rn{\rel} \dfeq \set{y \in Y}{\xsome{x}[X]{x \rel y}}$ (torej $\dd{\rel} \subseteq \cod(\rel)$).
			\end{itemize}
		\end{definicija}
	...
	\section{Funkcije kot funkcijske relacije}\label{RAZDELEK: Funkcije kot funkcijske relacije}
		...
		\begin{definicija}
			\df{Delna funkcija} je enolična dvojiška relacija.
		\end{definicija}
