\chapter{Relacije}\label{POGLAVJE: Relacije}
	...
	\section{Splošno o relacijah}
		...
		\begin{definicija}
			...
			\begin{itemize}
				...
				\item
					\df{zaloga vrednosti} ali \df{slika} relacije $\rel$ je množica $\rn{\rel} \dfeq \set{y \in Y}{\xsome{x}[X]{\rel[x][y]}}$ (torej $\dd{\rel} \subseteq \cod(\rel)$). \davorin{Morda dodamo kot možno ime še prevod angleške besede `range', se pravi `razpon'?}
			\end{itemize}
		\end{definicija}
	...
	\section{Relacije urejenosti}\label{RAZDELEK: Relacije urejenosti}
		...
		\davorin{Verjetno bi bilo dobro zadnje tri zadeve (razen dobre urejenosti?) dati v ločeno poglavje \qt{Strukturirane množice}, kjer povemo še več na to temo. Tja dodamo tudi Boolove mreže/kolobarje in povezavo z izjavno logiko.}
