\chapter{Relacije}\label{POGLAVJE: Relacije}
	...
	\section{Ekvivalenčne relacije in kvocientne množice}
		...
		Ko smo obravnavali bijekcije v \note{razdelku o bijekcijah}, smo omenili, zakaj je uporabno imeti obrate preslikav. Težava je seveda, da imajo samo bijekcije obrate (v smislu, da so tudi obrati preslikave --- kot relacije seveda imajo obrate), medtem ko včasih želimo obrniti tudi druge preslikave.
		
		Vzemimo na primer eksponentno funkcijo $\xlam{x}{e^x}$. Če jo obravnavamo kot preslikavo $\RR \to \RR$, seveda nima obrate, saj ni surjektivna. Ideja je, da zožimo kodomeno do zaloge vrednosti --- preslikave $\xlam{x}{e^x}\colon \RR \to \RR_{> 0}$ je bijektivna in posledično lahko definiramo njen obrat (naravni logaritem) $\ln\colon \RR_{> 0} \to \RR$.
		...
		\begin{izrek}[Naravna razčlenitev preslikave]
			...
			preslikava $i\colon \im(f) \hookrightarrow Y$ je vključitev zaloge vrednosti v kodomeno, preslikava $\tilde{f}\colon X/_\equ \to \im(f)$ pa je v celoti določena s pogojem
			\[\tilde{f}([x]) = f(x)\]
