...
\[f(0, x) = b(x)\]
in
\[f\big(\suc(n), x\big) = r\big(n, x, f(n, x)\big)\]
...
\begin{align*}
0 + m &\dfeq m \\
\suc(n) + m &\dfeq \suc(n + m)
\end{align*}
V tej definiciji $m$ nastopa kot parameter --- se pravi, uporabili bomo načelo parametrizirane rekurzije. Glede na oznake iz izreka~\ref{izrek:rekurzija} smo vzeli $X = \NN$, $Y = \NN$, $b(m) = m$ (torej je $b$ identiteta na $\NN$) in $r(n, m, k) = \suc(k)$ (se pravi, $r$ je kompozicija projekcije na tretjo komponento in preslikave naslednika). Po tem izreku dobimo enolično določeno preslikavo $+\colon \NN \times \NN \to \NN$ (ki igra vlogo preslikave $f$ iz izreka).
...
Seveda velja $0 + a = a$ za vse $a \in \NN$ --- to je del definicije seštevanja. Od tod pa ne smemo takoj sklepati na $a + 0 = a$, saj še nismo dokazali izmenljivosti seštevanja. Lahko bi na tem mestu začeli z dokazom izmenljivosti, ampak kot bomo videli, bomo za to že potrebovali dejstvo, da je $0$ enota. Dokažimo torej $a + 0 = a$ za vse $a \in \NN$ neposredno.

Trditev dokazujemo z indukcijo. Najprej dokažemo trditev za $a = 0$, torej $0 + 0 = 0$. To je res po definiciji.

Privzemimo, da velja $a + 0 = a$ za neki $a \in \NN$. Dokazujemo $\suc(a) + 0 = \suc(a)$. Preverimo:
\[\suc(a) + 0 = \suc(a + 0) = \suc(a).\]

Kaj pa, če namesto $0$ prištejemo $1$? Takrat seveda pričakujemo, da dobimo naslednika. Preverimo.

Za poljuben $a \in \NN$ dobimo $1 + a = \suc(0) + a = \suc(0 + a) = \suc(a)$. Tukaj sploh nismo potrebovali indukcije. Jo pa potrebujemo za dokaz, da za vsak $a \in \NN$ velja $a + 1 = \suc(a)$. Za $a = 0$ je to definicija oznake $1$. Recimo, da za neki $a \in \NN$ velja $a + 1 = \suc(a)$. Tedaj $\suc(a) + 1 = \suc(a + 1) = \suc(\suc(a))$.
...
Dokažimo jo z indukcijo (po spremenljivki $a$). Najprej vzamemo $a = 0$; tedaj za vse $b, c \in \NN$ velja $(0 + b) + c = b + c = 0 + (b + c)$. Privzemimo zdaj, da pri nekem $a \in \NN$ za vse $b, c \in \NN$ velja $(a + b) + c = a + (b + c)$. Pri poljubnem $b$ in $c$ lahko poračunamo
\[(\suc(a) + b) + c = \suc(a + b) + c = \suc((a + b) + c) = \suc(a + (b + c)) = \suc(a) + (b + c).\]

Zdaj lahko dokažemo izmenljivost (komutativnost) seštevanja. Dokazati želimo izjavo
\[\all{a \in \NN}\all{b \in \NN}{a + b = b + a}.\]
Spet dokazujemo z indukcijo. Vzemimo $a = 0$. Potem trdimo, da za vse $b \in \NN$ velja $0 + b = b + 0$. To smo že dokazali --- obe strani enakosti sta enaki $b$, saj vemo, da je $0$ enota za seštevanje.

Predpostavimo zdaj, da velja $\all{b \in \NN}{a + b = b + a}$ za neki $a \in \NN$. Trdimo $\all{b \in \NN}{\suc(a) + b = b + \suc(a)}$. Vzemimo poljuben $b \in \NN$ in preverimo:
\[\suc(a) + b = \suc(a + b) = \suc(b + a) = (b + a) + 1 = b + (a + 1) = b + \suc(a).\]
...
\begin{align*}
0 \cdot m &\dfeq 0 \\
\suc(n) \cdot m &\dfeq n \cdot m + m
\end{align*}
Če primerjamo z izrekom~\ref{izrek:rekurzija}, smo vzeli $X = Y = \NN$, $b(m) = 0$ (torej je $b$ konstantna ničelna preslikava) in $r(n, m, k) = k + m$ (to preslikavo lahko definiramo s pomočjo pravkar definiranega seštevanja). Izrek nam porodi enolično določeno preslikavo $\cdot\colon \NN \times \NN \to \NN$.

Podobno kot prej pri seštevanju za začetek ugotovimo, kaj se zgodi, ko množimo z $0$ oziroma $1$. Po definiciji vemo $0 \cdot a = 0$ za vse $a \in \NN$. Dokažimo še $a \cdot 0 = 0$ za vse $a \in \NN$. Za $a = 0$ velja $0 \cdot 0 = 0$ po definiciji. Vzemimo, da velja $a \cdot 0 = 0$ za neki $a \in \NN$. Tedaj $\suc(a) \cdot 0 = a \cdot 0 + 0 = 0 + 0 = 0$.

Število $1$ bi morala biti enota za množenje. Preverimo. Najprej $1 \cdot a = s(0) \cdot a = 0 \cdot a + a = 0 + a = a$. Po drugi strani trditev, da za vse $a \in \NN$ velja $a \cdot 1 = a$, dokažemo z indukcijo. Enakost $0 \cdot 1 = 0$ je jasna. Recimo, da trditev velja za neki $a \in \NN$. Tedaj $\suc(a) \cdot 1 = a \cdot 1 + 1 = a + 1 = \suc(a)$.

Dokažimo, da je množenje razčlenitveno (distributivno) glede na seštevanje. Se pravi, dokazati želimo izjavi
...
Pri prvi od izjav (desni razčlenitvi) za $a = 0$ dobimo $(0 + b) \cdot c = b \cdot c = 0 + b \cdot c = 0 \cdot c + b \cdot c$ in če predpostavimo ozjavo za neki $a$, lahko sklepamo $(\suc(a) + b) \cdot c = \suc(a + b) \cdot c = (a + b) \cdot c + c = a \cdot c + b \cdot c + c = a \cdot c + c + b \cdot c = \suc(a) \cdot c + b \cdot c$. Pri drugi izjavi (levi razčlenitvi) sklepamo podobno: $0 \cdot (b + c) = 0 = 0 + 0 = 0 \cdot b + 0 \cdot c$ in $\suc(a) \cdot (b + c) = a \cdot (b + c) + (b + c) = a \cdot b + a \cdot c + b + c = a \cdot b + b + a \cdot c + c = \suc(a) \cdot b + \suc(a) \cdot c$.

Preverimo družilnost množenja, torej izjavo
\[\all{a \in \NN}\all{b \in \NN}\all{c \in \NN}{(a \cdot b) \cdot c = a \cdot (b \cdot c)}.\]
Za $a = 0$ dobimo $(0 \cdot b) \cdot c = 0 \cdot c = 0 = 0 \cdot (b \cdot c)$. Nato predpostavimo zgornjo izjavo za neki $a$. Poračunamo $(\suc(a) \cdot b) \cdot c = (a \cdot b + b) \cdot c = (a \cdot b) \cdot c + b \cdot c = a \cdot (b \cdot c) + b \cdot c = \suc(a) \cdot (b \cdot c)$.

Naposled preverimo še izmenljivost množenja na naravnih številih, torej izjavo
\[\all{a \in \NN}\all{b \in \NN}{a \cdot b = b \cdot a}.\]
Za $a = 0$ dobimo $0 \cdot b = 0 = b \cdot 0$. Vzemimo, da izjava velja za neki $a$. Tedaj $\suc(a) \cdot b = a \cdot b + b = b \cdot a + b = b \cdot a + b \cdot 1 = b \cdot (a + 1) = b \cdot \suc(a)$.
...
\[\all{a \in \NN}\all{x \in \NN}\all{y \in \NN}\big(a + x = a + y \implies x = y\big).\]
Kot običajno se poslužimo indukcije. Pri $a = 0$ smo takoj na koncu. Recimo, da velja izjava za neki $a$ ter vzemimo poljubna $x$ in $y$, tako da velja $\suc(a) + x = \suc(a) + y$. Tedaj $\suc(a + x) = \suc(a + y)$ in ker je $\suc$ injektivna preslikava (eden od Peanovih aksiomov!), sklepamo $a + x = a + y$, od tod pa $x = y$.\footnote{Injektivnost preslikave $\suc$ je točno to, kar potrebujemo za krajšalnost. Velja namreč tudi obrat: če imamo $\suc(a) = \suc(b)$, tj.~$a + 1 = b + 1$, in lahko krajšamo, potem $a = b$.}
...
\end{align*}
Po načelu o neparametrizirani rekurziji dobimo enolično določeno preslikavo $\tilde{\prd}\colon \NN \to \NN$ (konkretno, v izreku~\ref{izrek:rekurzija} vzamemo $Y = \NN$, $b = 0$ in $r(n, m, k) = n$).\footnote{Morda se vam zdi vprašljivo, če bi to definicijo sploh imenovali ``rekurzivna'', saj $\tilde{\prd}\big(\suc(n)\big)$ nismo izrazili s $\tilde{\prd}(n)$ (ali z drugimi besedami, preslikava $r$ ni odvisna od svojega tretjega argumenta). Ampak izrek~\ref{izrek:rekurzija} za ta primer še vedno velja in zgornja definicija torej podaja dobro definirano preslikavo $\tilde{\prd}\colon \NN \to \NN$.}
...
Ideja prisekanega odštevanja je, da zmanjševanec zmanjšamo za tolikšen del odštevanca, kolikor le lahko (z drugimi besedami, če se običajno odštevanje izide v naravnih številih, velja $a \monus b = a - b$, sicer pa velja $a \monus b = 0$). Natančna rekurzivna definicija je sledeča.
\begin{align*}
m \monus 0 &\dfeq n \\
m \monus \suc(n) &\dfeq \tilde{\prd}(m \monus n)
\end{align*}

\davorin{Moram dokončati razpravo o prisekanem odštevanju (bo prišlo prav kasneje pri celih številih) in popraviti načelo rekurzije, da bo parameter na levi. To bo bolje tudi v drugih primerih.}
