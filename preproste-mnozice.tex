...
zakonom, ki mu pravimo \df{princip ekstenzionalnosti}: \davorin{princip = načelo}

\begin{pravilo}[Princip ekstenzionalnosti]
  Množici sta enaki, če vsebujeta iste elemente. \davorin{predlagam, da v definicijah namesto `če' uporabljamo `kadar', da ne bodo preveč izgledale kot implikacije}
\end{pravilo}
...
\begin{pravilo}
  ...
  natanko $x$ in $y$. \davorin{podajmo tako slovenske izraze kot tujke, torej tudi `(neurejeni) par'}
\end{pravilo}
...
\davorin{A dodamo, da obstaja neskončno enojcev, ampak (po načelu ekstenzionalnosti) samo ena prazna množica?}
...
%
\begin{itemize}
\item \emph{definiranost}: vsakemu elementu iz $X$ je prirejen vsaj en element iz $Y$, \davorin{prevajamo `total'; naj bo `celovitost'}
...
razlikovala. \davorin{Če se že spuščamo v to, potem dajmo opredeliti, kdaj \emph{sta} predpisa enaka. Na primer, ali sta enaka $x \mapsto 1 + x^2$ in $x\mapsto 1 + x \cdot x$? Kaj pa $x \mapsto \suc(0) + x^2$?}
...
\andrej{Kako bomo poslovenili ``aplikacija''?} \davorin{uporaba, izvedba (ta prevod premislimo skupaj z `elimination rule', da se ne bo prekrivalo)}
...
lahko \df{$f$ uporabimo na $x$} in dobimo \df{vrednost} funkcije~$f$ pri
\df{argumentu}~$x$ \davorin{vnosu?}, to je tisti edini element $Y$, ki ga~$f$ priredi~$x$. Vrednost $f$
pri~$x$ zapišemo
%
\begin{equation*}
  f(x)
...
Iz množic lahko na različne načine sestavimo ali \df{konstruiramo} nove množice.
Konstrukcijo opišemo z naslednjimi pravili:
%
\begin{itemize}
\item pravilo \df{formacije} pove \davorin{izoblikovanje, tvorba}
\item pravila \df{vpeljave} povedo, kaj 
\item pravila \df{uporabe}
\item \df{enačbe}
\end{itemize}
%
Kot prvi primer poglejmo \df{kartezični produkt}: \davorin{dodajmo tudi `zmnožek'}
%
\begin{itemize}
\item formacija: za vsaki množici $A$ in $B$ je tudi $A \times B$ množica, ki se imenuje \df{(kartezični) produkt} $A$ in $B$,
\item vpeljava: za vsak $a \in A$ in $b \in B$ lahko tvorimo element $(a, b) \in A \times B$, ki se imenuje \df{urejeni par}
...
\davorin{A bomo imeli makro za urejene večterice?}
