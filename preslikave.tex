\chapter{Preslikave}
	...
	\section{Bijektivnost in obratne preslikave}
		...
		\begin{dokaz}
			\begin{itemize}
				...
				\item\proven{$(2 \impl 1)$}
					Če je $f$ bijekcija, za vsak $y \in Y$ velja, da je $\pim[1]{f}{\set{y}}$ enojec (glej \note{ustrezne predhodne trditve}). Definirajmo $g\colon Y \to X$ na naslednji način: za vsak $y \in Y$ naj bo $g(y)$ tisti element $x \in X$, za katerega velja $\pim[1]{f}{\set{y}} = \set{x}$. \note{Iz lastnosti praslike sledi, da je $g$ obrat $f$.}
			\end{itemize}
		\end{dokaz}
		
		Iz dokaza te trditve vidimo, da bi bilo koristno imeti oznako za \qt{tisti edini element iz dane množice}, če želimo podajati tovrstne preslikave s simboli.
		
		\note{Preslikava $x \mapsto \set{x}$ je injektivna kot $X \to \pst(X)$ in obrnljiva, če zožimo kodomeno na enojce. Uvedba jote.}